...
it somehow end up in malicious hands, either by a device being lost or a person
hacking in to a device. Even though it is encrypted it might be weak to a brute
force or dictionary attack. This section explains the procedure taken by us
attack a keyring.

\subsubsection{Impact}
If an attacker is able to decrypt the keyring he will obtain the information to
access a users root folder. This is potentially enough for the attacker to
access every file stored by the user. 
...
guessed. The decryption involves both key derivation with \ac{PBKDF2} and decryption
with 128-bit \ac{AES} in \ac{ECB} mode. We verified a successful attack by our
knowledge of the serialization of the Capability.
...
perform dictionary attacks by a cluster of cooperating computers. Both use
...
Jurisdiction Policy files installed.  It also requires Java to be configure to
use \ac{BC} as its primary \ac{JCE} provider. The policy files and \ac{BC}
provider are required because the keyring is encrypted using BC version
1.34\footnote{BC version 1.34 is used by default in Android 2.2.}.\\
%TODO: Confirm that BC v.1.34 is default!!!1

\noindent \ac{CDA} requires almost the same configuration as the BruteForce
attack, but for each node in the cluster. Another important requirement for
...
BruteForce attack  we ran is given in Table \ref{tab:hwbf}.The \ac{CDA} attack
was executed over a cluster of Amazon \ac{EC2} instances. All instances used
were categorized by Amazon as ``high-CPU extra large instances``. The hardware
specification for a single instance, used in the \ac{CDA} attack, is given in
Table \ref{tab:hwcda}. We used 20 of these instances.
...
The command used for executing a dictionary attack with the tool BruteForce
can be seen in Listing \ref{lst:bf}. The parameter \textbf{m} is the maximum
password length to try, and the \textbf{t} parameter is the number of threads to
execute.
...
The local dictionary attack was also executed by the tool BruteForce. The
command used to execute this is shown in Listing \ref{lst:da}, where the
parameter \textbf{t} is the number of threads to execute.
...
desired dictionary file and keyring into the \ac{HDFS}. Copying files from the master
node to the \ac{HDFS} is done with the following command shown in Listing
\ref{lst:cpHDFS}. The attack can then be started on the master node with the
command shown in Listing \ref{lst:CDA}. The parameter \textbf{s} is the number
of slaves to use in the cluser, \textbf{t} is the number of threads to execute
per slave. In our attack we used 20 nodes with 8 threads each.
