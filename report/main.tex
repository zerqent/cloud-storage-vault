...
Capabilities are stored serverside in folders, in it's serialized form shown i
figure \ref{fig:CAP:serial}. Object Type specifices if the capability
represents a file or a folder, with values F or D (for directory) accordingly.
Key Type specifices the permissions the key will give on the object and can be
either RO (Read Only), RW (Read and write) or V (Verify). The different parts
are separated by the character ``:``. Key and verify are encoded in Base32,
which means that the 128 bits will be repleaced by an alphabet of 32
different symbols, namely A-Z and 2-7. This transformation is done for human
readability. 
...
CSVFileManager.java providies specific methods for uploading the encrypted
objects.
...
to manually copying the key from one users client to anothers, and afterwards
verifying that the key for the selected folder is ok. The other possiblility is
to make use of a barcode. What this means is that one user will generate a
barcode on his device, and the other user can scan that code using his device'
camera. The barcode contains both the key and verification for the shared
...
take any other folder and use as a the root for the new folder if that is the
users wish.
