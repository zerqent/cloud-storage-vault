...
This thesis deals with this question and suggest a scheme to make data stored
...
or more properties towards being, we will define it, a completely secure cloud
...
Finally, it is important that the implementation of the cryptographic scheme is
available as open source. This because the security should be verifiable to
anyone interested in using the software. In addition, open source encourages
participation and analysis of the system which in turn will lead to a more
secure system.
...
parent directory for future usage. She can then download shared files as if they
where her own.
...
\paragraph{Encrypted Keyring} Every user will need to possess a root
capability to access their files. Due to the amount and randomness of data in a
...
the camera on their device. Figure \ref{fig:CSVAndroid:barcode} illustrates how
...
in the client. To make the \ac{GUI} even easier to understand, we have created a
video that examines the most important user interactions with the client
implementation. The video is available in the attachment, which is further
described in Appendix \ref{ap:attachments}.

In addition to the attached video, this section will scrutinize the most
important parts of the implemented \ac{GUI}. 
...
have to unlock her encrypted keyring with her password. If it is the first time the
user starts the application, she will have to enter her online credentials, and
...
Due to the risk of an external attacker, a secure communication channel is
needed between the client and the server.
