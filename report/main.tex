...
Authentication is the challenge of a user to prove that he is who he claims to
be. In a real world setting this might be accomplished by the use of an ID-card
or a passport, in a digitial world this is usually solved by the user
knowing, having or beeing something or a combination. What a user knows might
be a password, what he has may be his cellphone number and being might be his
fingerprints.
% FIXME: Må også reflektere at det han autentiserer mot må kunne verifisere
...
verifies that an entity aprooves with or has written something. An easy example
is that a party has written a message he sends it through a cryptographic hash
function and encrypts it with his private key. A receiving party can then hash
the same message and decrypt the signature using the users public key and
verify that the two hashes match. With this scheme we have obtained
authentication, integrity and non-repudiation for the message, given that we
already know that the public key for the sending user is held by the person we
believe the message originates from.
