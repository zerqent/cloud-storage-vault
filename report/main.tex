...
Java library available at \cite{pbkdf2}. The program is divided into two attack
functions named bruteForceAttack and dictionaryAttack. The bruteForceAttack
function is shown in Listing \ref{lst:bffunc}.
...
argument. The bruteForceAttack function is based on letting the main thread
push words onto a stack of words, while the remaining BruteForceThreads are
popping words from the stack. The BruteForceThreads are further trying to
decrypt the encrypted keyring by using each word as a possible password. In this
way, the main thread and all of the BruteForceThreads are simultaneously
cooperating to find the correct password. Once a BruteForceThread finds the
correct password, the program will print out the password and exit.\\
