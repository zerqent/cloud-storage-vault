...
the trust of the provider. This includes secure sharing of private data among
users of the same service.

The underlying cryptographic architecture is based on existing open source
systems and cryptographic primitives. The architecture was implemented as a
reusable general library in Java. An Android client was created, and several
performance tests were conducted\footnote{Source code available at
...
user experience on an Android device. Possible weaknesses of the scheme are
identified.
...
\paragraph{Chapter \ref{ch:futurework} -- Future Work} suggests possible
enhancements and additional features that are applicable to the proposed
cryptographic scheme.

\paragraph{Chapter \ref{ch:conclusion} -- Conclusion} extracts
...
For example, a user has a certificate with some clues about an identity in
...
even higher unknown speed. The same researcher has also hinted that
...
randomly generated. The key is is derived from a password using \ac{PBKDF2}. The
weakness of this, compared to a random 128 bit key, is that users probably
chose shorter and less random passwords.

Another weakness is that parts of the keyring plaintext are known by an
attacker. The serialization of a capability for a writeable folder will start with
...
take about 289 283 years in average to recover a password with the 10
...
set of passwords, the average time will represent the time used to check
...
This chapter will discuss the technical procedure elaborated in Chapter
\ref{ch:technical}, as well as experimental findings from Chapter
\ref{ch:results}. 

The security of the cryptographic scheme will be analysed in relation to the
applicable security services. Regarding the implementation, we will look at the
choice of use case and the corresponding choice of key distribution. We will
additionally discuss the performance and security findings associated with the
implementation.

\section{Security of the Cryptographic Scheme}
...
\subsection{Security of the Client}
...
attacks. The vulnerability of the keyring against these attacks was questioned
in Section \ref{sec:BFLK}.
...
secured against such an attack, it is still vulnerable to dictionary
...
with the speed obtained by Roth, such an attack seems infeasible, while a
...
\section{Simplifying the Server}
...
devices, that has further been measured in performance and evaluated in
...
\emph{\ac{BFDA}} includes a brute force attack and a dictionary attack against
