...
in the cloud secure. We define the criteria of a secure storage system by the
following:
...
should be available as open source and well documented. This because the
security should be verifiable to anyone interested in using the software. In
addition, we believe open source encourages participation and analysis of the
system which in turn will lead to a more secure system.
...
with which is called the \emph{\ac{ECB}} mode of operation. The problem with
...
privacy are not considered as they are not topical to our research.
...
The \emph{key-value store}, is the lowest layer and is implemented by a grid
...
A user will have to have some way of storing the capabilities of her file
...
are not really any reason to not select a key size of 256 bit \cite{schneier}.
...
communicating over \ac{TLS}. As this is a technology under continuous
scrutiny, we recommend setting the available parameters as high as possible to
meet the demands of the current situation. The reason for not specifying this
more thorough, is that libraries and software used both server- and client-side
...
of \ac{PBKDF2} is quite similar to the one in our scheme. It uses 4096
iterations, and H\ac{MAC}-\ac{SHA}-1 as the pseudorandom function. So far, the
most effective brute force attack against \ac{WPA} was published on Black Hat
DC 2011 by a security researcher named Thomas Roth. He proved that anyone can
...
4096 iterations, and increase the number if this is too fast. The pseudorandom
function should be H\ac{MAC}-\ac{SHA}-256, since \ac{NIST} recommends against
utilizing \ac{SHA}-1 in new applications. The salt should be at least 128 bits.
...
folder, and have the information to read, write and verify that object.
...
a video that examines the most important user interactions with the client
implementation. The video is available in the attachment, which is further
...
the user starts the application, she have to enter her online credentials, and
get a choice to either import an existing root folder, or to generate a new
...
\emph{Browse the Vault} -- in other words to see the files that the user has
...
measure the security of the locally encrypted keyring.
...
computer. We emphasize that the measurements are meant to be taken as
...
the keyring, she will obtain enough information to access a user's root folder,
...
\ac{ECB} mode. \ac{PBKDF2} is a function that can be used with a
...
presented that target the encrypted keyring.
...
implementation elaborated in Chapter \ref{ch:technical} as well as experimental
findings in Chapter \ref{ch:results}. 

We will scrutinize the cryptographic scheme for its level of security and solid
features. Suggestions to support additional features are also considered.
...
channel, encrypted and authenticated, for communication is not used.
...
session. The provider will also see which objects the users stores is a folder
and which is a file, because a write enabler is sent alongside a folder. The
sequence of objects that a user requests will also leak some information about
the file and folder hierarchy. We do not consider any of this information to be
dangerous for the server to possess, but if an external attacker obtain the
Write key for a folder this might cause some problems in regards to
availability.

\subsubsection{Confidentiality}

Confidentiality is provided by encrypting data locally before it is stored in
the cloud. This implies that confidentiality can only be breached by weaknesses
in the cryptographic primitives, side-channel attacks or attacks on the client.
...
\ref{sec:cryptoprimitivechoice} are believed to ensure confidentiality.
It is also possible to exchange the recommended alternative cryptographic primitives
with future or existing algorithms, confidentiality will then rest on the new
primitives.

The proposed cryptographic scheme has an additional weakness that can possibly
end with a breach of confidentiality. This is the procedure taken to derive the
...
corresponding public key, it will be easy for an attacker to retrieve the
folder's write key. While the cryptographic primitives recommended should make
this infeasible to do at the current time, it might be possible in the future.
The write key could instead be generated from a \ac{CSPRNG}, which is the case
for the read key of files.
...
The cryptographic scheme provides integrity by adding a cryptographic hash value
into a cloud object's encrypted parent folder that is stored in the cloud. The
hash value from a cloud file is derived from its content, while the hash value
from a cloud folder is generated from its public key. To breach integrity, an
attacker must be able to change a cryptographic hash value stored
inside a cloud folder. This implies that both reading and writing to a cloud
folder is achieved.

With this in mind, the integrity of the cryptographic scheme relies on the
confidentiality provided by the encrypted cloud folders. In other words, the
integrity relies on the underlying cryptographic primitives as well as the
client security.
...
unlinked a file to the folder. The scheme does unfortunately not provide the
information necessary to confirm which user changed the folder or when it
happened. It is possible, however, to implement this functionality on top of the
proposed scheme to provide non-repudiation. 

%It is worth noticing that enabling non-repudiation will enable the cloud
%provider to log user specific activities. Given a non-trusted cloud provider,
%one can not guarantee that the logged activities are correct.
...
indicate possession of the capability, but this value can theoretically be
stolen by using traffic analysis if the communication channel is not secured.
The possession of a capability does authentication in some way, but not of the
...
Access Control to a resource in the proposed scheme is given by capabilities.
...
risk of this is to have the server actually verify that folders uploaded have a
correct signature. With that improvement the write enabler is strictly not
...
If a secure communication channel is not used availability will be a problem in
regards to an external attacker as well. By using traffic analysis Write
enablers for folders can be obtained, which means that the data can also be
overwritten. A \ac{MITM} attack could use modification of message to either
prevent a user from storing data or to make the user store wrong data. These
attacks do not compromise the confidentiality of the data, and they will be
discovered when the attacker goes away and the integrity check will fail.  A
possible attack, that is not detected by the integrity check, is the replay
attack. The attacker can record an update of a folder and replay that message
when the folder has further been updated causing possible loss of data.
...
\subsection{Improving the Scheme}
...
\subsubsection{Deletion of Files}
...
\subsubsection{Verification of Files}
...
this way a key can almost always be ready for the user when she needs it.
...
attack against the encrypted keyring. By doubling the number of iterations in
