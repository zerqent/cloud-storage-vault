...
\begin{tabbing}
    ...
    External Supervisors: \> Carsten Maartmann-Moe and \\
...
signatures. These primitives needs to be set in accordance with the security
requirements established in TODO. The hard part of selecting appropriate
cryptographic primitives is trying to predict how long the primitives will be
secure. Keylength.com\footnote{\url{http://www.keylength.com}} list predictions
on how long primitives will be secure based on different sources with different
predictions.
...
standard for symmetric encryption and is readily available in most programming
languages. We have chosen a key size of 128-bit. ECRYPT II has one of the more
pessimistic predictions on how long this will be secure, saying data should be
secure until 2030-2040\cite{ecrypt2_2010}. The gain of not choosing a larger
key is a somewhat greater performance\footnote{\ac{AES}-128 is 10 rounds while
\ac{AES}-256 is 14 rounds} and of course that the keys are smaller to store.

\paragraph{Mode of operation} The mode of operation we have chosen is \ac{CBC}.
This is based more on practical advice than on security consideration. Bruce
Schneier advices the use of either \ac{CBC} or \ac{CTR}-mode, where
\ac{CBC}-mode is easier to implement correctly \cite{schneier}. 

\paragraph{Padding} One negative consequence of \ac{CBC} is that \ac{CBC}
requires that the plaintext is an exact multiple of the block length, 128-bit.
Since this is not always the case a padding scheme will also be required. A
padding scheme does not have any security implications as long as it is
reversible\cite{schneier} at least not for \ac{CBC}. We went with PKCS5Padding
in our implementation based on its existence in the cryptographic libraries we
use.

\subsubsection{Cryptographic Hash Function}

The \ac{SHA}-family is the current standard for cryptographic hash functions,
and from this family we have gone with double \ac{SHA}-256. The cryptographic
scheme requires the hash function to have an output of at least the size of the
key used for encryption. \ac{SHA}-1 have an output of 160-bits and could have
been used to this but have weaknesses and are not recommended for use in new
systems. The use of double \ac{SHA}-256 compared to single is to prevent a
length-extension-attack\cite{schneier}.
...
Tahoe-\ac{LAFS} made the same choice. There might however be a performance
bonus in selecting \ac{RSA}. An internet draft suggests that \ac{DSA} is about three
times faster than \ac{RSA} at signing, but \ac{RSA} is about ten times faster
at verifying a signature\cite{dsa_sha2}. A performance comparison from Microsoft
suggest that \ac{DSA} is 29\% faster at signing and \ac{RSA} is 29\% faster at
verifying signatures\cite{msdn_perf}. Verification, opening a folder, is an
operation we believe most users will do significantly more than signing,
updating and creating folders. 
...
% Hash trees, server involvement, streaming
The verification scheme for files is suboptimal in pretty much every way a user
wants to use a file, with the exception of the case where the user wants the
entire file and the file has not been tampered with. The problem with the
scheme is that the user will have to download the whole file before he is sure
the file is OK. If it turns out the file is not OK, and the error was the first
byte, the user has wasted time and bandwidth downloading a lot of data he will
not need. Another problem is that if the file is for instance a movie file, and
the user only wants to look at a small part of this file. There is no way for
the user to verify only parts of a file, he will have to download the whole
thing which is suboptimal. A solution to this would be to build a hash-tree of
the whole file, as shown in Figure \ref{fig:hashtree}. With a hash-tree smaller
bits of the file can be verified, and errors can be detected earlier. The
hash-tree could be stored encrypted on the server together with the ciphertext,
and the capability could hold only information to verify the hash-tree. The
downside to this approach is that performance would be affected due to the
substantial amount of hash operations required to do this.

\begin{figure}[!h]
\centering
\includegraphics[scale=0.55]{hash-tree.pdf}
\caption{Hash-tree for a file}
\label{fig:hashtree}
\end{figure}


\subsection{De-duplication}
% How Tahoe does this, security vs. performance trade off 
...
probability that a certain user is legit. The reason why we do not want this
scheme is that it is fairly complex, and to easy for users to make mistakes
when using it.  

If our cryptographic scheme should be used in an organization a
\ac{PKI} is probably the best solution. In this setting the organization can
itself be the trusted third party, who enforces that all certificates granted
to users are correct. A user could then be simply prompted by the name of the
user she would like to share a file with, and the rest could be handled by
logic and the trust of the \ac{PKI}.
...
This section will discuss the general security provided by the Cloud Storage vault. The
security of the Cloud Storage Vault is divided into security of the cryptographic
scheme and security of the user client.
...
Additionally, a user's client might also be broken into without the user's knowledge.
...
educated attacker to reveal the user's server access password as well as the
password to decrypt the locally stored keyring. The attacker will then have access to the
user's files and folders at any time. The attacker can achieve the passwords by
...
flush the password values right after a successful login session. 

Even though this prevents disclosure of critical access data, it does not prevent
a logged in attacker from disclosing the user's encrypted files and folders.
However, to solve this problem the Cloud Storage Vault should implement a
...
the application will reduce the consequence of losing a user client. It is
important that the timeout is set to a value between what is acceptable by users
and what can be defined as secure enough\footnote{Users would not like to write
...
The attacker can additionally utilize software to read from the client's
memory.
%TIMEOUT VALUE
%SLETTE FILER PÅ DISK (TMPFS)

\subsubsection{Security of Encrypted Keyring}
In both cases above, with a broken or lost user client, an attacker can
copy/achieve the encrypted keyring stored by the Cloud Storage Vault.
%TODO: write intro
...
only accessible by its corresponding user.
