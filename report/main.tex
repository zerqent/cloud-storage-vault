...
(e.g. file upload, key generation) extensively, to prevent the interface from
hanging and to deliver a smoother user experience in general.  
...
%
The source of our client is logically separated into two entities; CSVlib
and CSVAndroid. CSVlib is a pure Java library which contain the necessary
entities, cryptographic operations and communication required for the client.
CSVAndroid contains primarily a graphical user interface to use CSVlib on an
Android device.

\subsubsection{Cryptographic Entities}
All the cryptographic entities, namely folders, files and capabilities are all
part of CSVlib. Their relationship can be seen in Figure
...
\paragraph{Capabilities} The implementation of Capabilities is a container for
cryptographic keys and information to identify the corresponding object. A
...
in Figure \ref{fig:CAP:serial}. Object Type specifies if the capability
represents a file or a folder, with values F or D (for file and directory)
accordingly.  Key Type specifies the permissions the key will give on the
object and can be either RO (Read Only), RW (Read and write) or V (Verify). The
different parts are separated by the character ``:``. Key and verify are
encoded in Base32, which means that the 128 bits will be replaced by an
alphabet of 32 different symbols, namely A-Z and 2-7. This transformation will
give some overhead in storage (26 bytes compared to 16), but makes it possible
to read for a human with few mistakes or misunderstandings.
...
\paragraph{Folders}
% TODO: Ref til crypt solution
A folder is represented in the class CSVFolder. A CSVFolder object is
just a collection of aliases and their corresponding capabilities. For
the most part the data stored in a folder is so small that it can easily live for 
as long as needed in the memory of the client. When a folder is created or 
updated the content is serialized and encrypted before it is uploaded to the server 
directly from memory. The serialization for each item is a folder is simply
alias;capability, where the capability is also serialized.

\paragraph{Files}
A file is represented in the class CSVFile. While a folder in general is small,
a file can be of any size, even larger than the \ac{RAM} of the device itself. To
keep the memory footprint low we use a pipe-and-filter architecture to stream
data all the way from the cloud to the disk, or vice versa, through encryption
and verification. An example of how we do this for uploads is shown in Listing
\ref{lst:inputstream}. The InputStreams are chained together, with the effect
that a read from ``readBuffer`` will trigger a read trough the whole pipe. The
DigestInputStream will update the state of the hash function, but is
transparent in the sense that whatever goes into the stream will also be what
comes out. The CipherInputStream on the otter hand will output an encrypted
stream of the data from the file. Buffers are placed between each step of the
stream for some performance increase.  

\lstinputlisting[label=lst:inputstream, caption=Pipe and filter upload of a
file]
...
HttpClient. This Client offers support for authenticated requests to a server,
and both upload and download through {\bf PUT} and {\bf GET} requests
respectively. We wrap communication with the server in two classes,
Communication.java and CSVFileManager.java. Communication.java provides
functionality for sending and retrieving data from our server while
CSVFileManager.java provides specific methods for sending and receiving the
encrypted objects, CSVFile and CSVFolder.
...
A shared folder is basically just a folder object which two or more people have
the required encryption keys for. However the problem of creating a share with
someone is that you have to verify that you are actually sharing with the
correct person. The data that will have to be shared is a serialized form of
the capability, shown in Figure \ref{fig:CAP:serial}. The client we have
implemented supports two methods of doing this. The most cumbersome is having
to manually copying the key from one users client to another, and afterwards
verifying that the key for the selected folder is correct. An example of this
can be seen in Figure \ref{fig:CSVAndroid:manualimport}. This feature is also
needed to support out-of-band methods for establishing a share. The other
possibility is to make use of a \ac{QR} code, which is a matrix barcode that
can store information. What this means is that one user will generate a barcode
on his device, and the other user can scan that code using the camera on his
device. Figure \ref{fig:CSVAndroid:barcode} displays how this code will look.
The barcode contains both the key and verification for the shared folder. Once
two users have shared a folder once, that folder can be used for all future
shares which means that the two people will never have to meet and do the
capability exchange again.
...
that needs to be transfered is that of the root-folder. It is also possible to
take any other folder and use as the root for the new folder if that is the
users wish.
...
be stolen or broken into by some means, if the capability is stored in clear
text it is easily stolen. We partially remedy this by the use of \ac{PBKDF2},
which makes an encryption key from a password. This is however no defense if
the attacker managed to read the memory while the key is unlocked. The client
enforces that this password should be at least 9 characters long. 
