...
\ac{PBKDF2} is a key derivation function which uses key stretching. It uses the
...
A higher iteration count results in a stronger key.
...
individual operations in the pipeline on the HTC Desire.
...
higher for all three devices. From the individual operation results for HTC
Desire we note that the encryption and decryption part is the time consuming
operation.
...
that lead to an Out of Memory exception during execution.
...
and signing is pretty fast for all devices. We also note that our
implementation struggle to handle folders with large contents on the HTC Hero.
