...
directory's \emph{storage index} is further added as a hash value of the directory's
read key. The next value added to the directory is the \emph{\ac{IV}}. The \ac{IV} is
generated from a counter value stored at the user terminal. It is used to
prevent predictable cipher text, when the directory entries are encrypted.  The
next value added to the directory is the \emph{write enabler}. The write enabler is an
HMAC-SHA-2 value created from the directory's write key and a static chosen text
value. It is used by the ACL layer on the server side to check that a user has
write permission to upload the directory.
