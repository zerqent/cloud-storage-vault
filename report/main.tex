...
\subsection{Encryption} Encryption is the process of transforming some
...
self and data in form of entries, as per described in Table
\ref{tbl:folder:contents}.
...
the directory must be created locally and uploaded from the user terminal. Second, the directory must
further be accepted on the server side. Both parts are described below. Creating
an uploading a directory from the user terminal is illustrated in Figure
\ref{fig:CS:CD}.
...
Before creating a new directory it is necessary to generate a cryptographic
key-pair. This key-pair is specific to the new directory and consists of a
public and private signing key. The signing key is further used to derive
a write and read key for its corresponding directory. How keys are generated and
derived is explained in Section \ref{sec:CS:KAC}.
...
(IV). The IV is generated from a counter value stored at the user terminal. It
...
text value. It is used by the ACL layer on the server side to check that a
user has write permission to upload the directory. Accepting an upload on the
server side is shown in Figure \ref{fig:}.
...
permission. It is therefore encrypted with the directory's write key before it
is added to the directory. The signing key is encrypted using AES in CBC mode.
The next value added is the directory's entries. The entries of a
directory should only be visible to users with read permissions to
the directory. They are therefore encrypted with the directory's IV and read
key before they are added to the directory. The entries are encrypted using AES
in CBC mode. The last value added to the directory is a signed hash value of
encrypted entries. The hash value is encrypted with the signing key using RSA.

After creating a new directory, the directory is further uploaded to the remote
server and checked for acceptance.

Accepting a directory on the server side is illustrated in Figure \ref{fig:}.
...
A modified directory must contain a valid signature to prove that it has been
