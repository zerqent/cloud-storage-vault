...
cryptographic hash function, a \ac{MAC} function and a function for digital
signatures, as found in Figures \ref{fig:CS:CF}, \ref{fig:CS:OF},
\ref{fig:CS:CD}, \ref{fig:CS:VOD} and \ref{fig:CS:OD}. 
...
but more and more attacks are discovered (e.g. \citet{sha1}) and are not
...
We recommend \ac{PBKDF2} as the function to derive cryptographic keys from human memorable
passwords. The standard for \ac{PBKDF2} was written by \ac{RSA} Laboratories in 1999
\cite{PBKDF2std}. \ac{PBKDF2} has further been recommended by \ac{NIST} in \cite{pbkdf_nist}.

As mentioned earlier, \ac{PBKDF2} utilizes a salt during key derivation. The
purpose of the salt is to prevent general attacks using precomputed rainbow tables
to crack the input password. The salt is recommended by \ac{NIST} to be at least
128 bits in length.

In addition to setting a salt value, \ac{PBKDF2} provides the possibility of
adjusting the number of iterations used in its inner logic. The number of
iterations defines the computational workload of the key stretching process, and
is mainly used to resist brute force and dictionary attacks. It is therefore
proposed that the number of iterations chosen should be set as high as possible,
while maintaining acceptable performance \cite{pbkdf_nist}.

To come up with a secure recommendation, we looked at the usage of \ac{PBKDF2}
in WPA\footnote{WPA is a security protocol for wireless networks} and what
security it provides against brute force attacks. WPA is an excellent measuring
unit, since the utilization of \ac{PBKDF2} is quiet similar to the one in our scheme.

So far, the most sensational brute force attack against WPA was published on
Black Hat DC 2011 by a security researcher named Thomas Roth. He proved that
anyone can crack WPA passwords with a speed of up to 400 000 passwords per
second, using multiple Amazon \ac{EC2} cluster \ac{GPU} instances \cite{rothwpa}. His
findings also indicate that Amazon them selves can reach an even higher unknown
brute force speed.

With this in mind, and that WPA uses 4096 iterations in \ac{PBKDF2}, we
recommend to use at least 4000 \ac{PBKDF2} iterations for the proposed
cryptographic scheme.

The recommended number of iterations provides an acceptable performance, but
makes the cryptographic scheme vulnerable to heavy brute force attacks. 

\paragraph{Password requirements}To avoid the most extreme brute force attacks,
we suggest that the following password requirements are complied with.
...
root capability in a file stored on the client. We call this the encrypted key
ring.
...
measurements on the proof of concept client, and figures on the attacks
presented that target the local keyring.

\section{Client Performance}

This section presents the results of benchmarking of the proof of concept
...
\ref{fig:bfres}. The histogram in Figure \ref{fig:bfres} shows how many
passwords per second we were able to brute force versus the different amounts of
iterations used in PBKDF2.
...
Using 1000 iterations in PBKDF2, as recommended by RSA Laboratories,
the brute force attack was able to reach a speed of around 1800 passwords per
second. With this speed it will take, in average, 24 hours to crack a password of
length 6, consisting of only small letters.
...
per second more compared to an instance in the cluster. This was expected, with
respect to the difference in CPU power between the local computer and a single
cluster instance \cite{cpubench}. 

With 20 nodes in the cluster we were able to brute force around 35 000 passwords
per second, given \emph{PBKDF2} with 1000 iterations. Running the CDA in a
cluster of 200 nodes, would further reach a speed of 350 000 passwords per
second. The latter case would, in average, use about 7 minutes and 20 seconds to crack a
password of length 6, consisting of only small letters.

Renting a single Amazon EC2 high-CPU extra large instance costs 0.68 USD per
hour. This unit price results in an hourly cost of 13.6 and 136 USD for a
clusters of 20 and 200 instances respectively. 
...
about 50-80\% on all three devices. There can be many reasons why this happens.
For the Android devices the most probably cause is that either the \ac{AES}
implementation in Dalvik is to slow, the hardware is to slow or our
optimization is slow. Files travel through the pipeline which is worked in a
single thread.  The problem with this is that we have to wait at several
stages, for the memory card to read, the hash function to update its state and
the AES block for encrypting. A multi threaded approach would probably be
favorable here, where one thread continiously reads data from disk, one
encrypts, one hashes and another one writes to the network. This option would
probably have even greater affect on the computer, due to its multi core
architecture. 
...
the generation of new cryptographic keys. We do not believe this is a big
problem for the end user, since he will not create that many folders in
sequence, and the maximum two seconds it takes to create a folder can easily be
done as an asynchronous task. The other cryptographic operations on folders
runs quite quickly, but the performance for serializing the folder is painfully
low when folders reach a large size. Now a folder with thousands of children is
probably not something a normal user will have, but in any case it's the
serialization in our client that takes time. In our client the content of a
folder is implemented as a hashmap, and the serialization is performed by
transforming all of these contents to a string, which is then encrypted. This
is probably not the fastest implementation possible. All data that should be
encrypted is already in memory so in an optimal implementation the
serialization phase should take almost no time, since the data that has to be
encrypted could be read directly from it's current location. 
...
difference in size between folder and file objects. A user's root folder is also
known since it is the first object accessed in a user session. It is also
possible for the provider to reveal the structure of the filesystem. The series
of file and folder requests from a user enables the provider to learn which
files are contained in which folders.
...
manages to obtain the asymmetric private key belonging to the folder. In other words, a
...
\subsubsection{Disclosure of Keyring Content}
The confidentiality of the cryptographic root key and user passwords is held by
a single encrypted keyring that is stored locally for each user client. The
...
vulnerability against brute force and dictionary attacks was experienced in
Section \ref{sec:BFLK}. 
%TODO: The paragraph below will discuss our findings from Section \ref{sec:R:BFLK}.


\paragraph{Brute Forcing the Keyring}
The confidentiality of the root folder can be breached by brute forcing a user's
locally stored keyring. This was experienced in Section \ref{sec:BFLK}. 
...
With the password requirements above and with 4000 iterations in
\emph{PBKDF2}, it will be extremely hard to perform a plain brute force attack against the local
keyring. 
...
and preventive action would be to simply ensure that the encrypted keyring is
only accessible by its authorized users.
