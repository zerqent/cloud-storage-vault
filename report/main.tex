...
in Figure \ref{fig:AS:filesystem}, and users can
link in folders and files as wanted. This provides a flexible and
space-conservative structure that is easily extendible.

Each client keeps a local copy of the write capability and the identity to the
users root folder in a password protected keyring. In the event of loss of
one of the clients, a user can change encryption keys (and identifier) for the
root folder.

The challenge with using services from 
% TODO:
% - Various File access modes (RO/RW)
% - Multiple clients, different entries to root folder
