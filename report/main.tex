...
(e.g. file upload, key generation) extensivly, to prevent the interface from
...
The implementation of Capabilities lives in CSVlib and is a container for
...
Capabilities are stored server side in folders, in it's serialized form shown i
figure \ref{fig:CAP:serial}. Object Type specifies if the capability
...
A folder is represented in the class CSVFolder. Basically a CSVFolder object is
...
functions for sending and retrieving data from our servers while
CSVFileManager.java provides specific methods for uploading the encrypted
objects.

\subsection{Sharing}
% Sequence Diagram - How we use the cryptographic solutions (and which ones) to
% achieve sharing, securely
% TODO: Ref til hvor vi beskriver sharing
...
someone is that you have to verify that you are actually sharing with the same
person. The data that will have to be shared is a serialized form of the
capability, shown in Figure \ref{fig:CAP:serial}. The client we have
implemented supports two methods of doing this. The most cumbersome is having
to manually copying the key from one users client to another's, and afterwards
verifying that the key for the selected folder is correct. The other possibility is
to make use of a barcode. What this means is that one user will generate a
barcode on his device, and the other user can scan that code using the camera
on his device. The barcode contains both the key and verification for the shared
folder. Once two users have shared one folder they will never have to meet
again to share something else.
%% TODO: Sjekk device'
