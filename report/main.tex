...
The server, in the most basic form, has to support two operations -- upload
({\verb PUT }) and download ({\verb GET }). In addition, an \ac{ACL} layer is
needed to support user management and the deletion of files.

The Python programming language was chosen as development platform, together
with a set of applications, interfaces and micro frameworks. The rationale for
each of these follows.

% FIXME: Can someone with English skills correct this sentence? :-)
\paragraph{Python} Python is a high-level general-purpose programming language.
It was chosen to program in due to previous knowledge and experience by the
...
software, and has been so since 1996.
...
module, namely {\verb mod_wsgi }, and the application is where we put our
code.

For each of the requests the server (i.e. {\verb mod_wsgi }) receives, a call to
the application function is made with two arguments -- a data structure
containing the environment variables, and a callback function for which the
application uses to return data to the requesting user.
...
of {\verb GET } and {\verb POST } requests.

% Tech: Apache/WSGI/Python/Pyroutes
%
\begin{figure}[h!]
    \centering
\begin{verbatim}
