...
\paragraph{Encrypted Keyring} Every user will need to keep a copy of his root
capability somewhere. The capability grants data to all of the users files, and
must therefore be stored securely.
...
A user will need to possess his root capability to be able to reach his files,
and due to the amount of information in a capability this will mean that the
user terminal will have to store the capability. The amount of information is
dependant on the sizes of the cryptographic keys used, but with a minimum key
size of 128 bit, this is already to much for most people to remember.

It is therefore recommended that the keyring is encrypted in some form, and a
...
attacks.  \ac{WPA} is an excellent measuring unit, since the utilization of
\ac{PBKDF2} is quite similar to the one in our scheme. It uses 4096 iterations,
and \ac{HMAC}-\ac{SHA}-1 as the pseudorandom function. So far, the most
sensational brute force attack against \ac{WPA} was published on Black Hat DC
...
he might be able to reach 1 million keys per second on the same equipment
\cite{rothblog}.
...
utilizing \ac{SHA}-1 in new applications.

\paragraph{Password requirements}

\ac{NIST} also recommends that passwords should be at least 10 characters. If
we assume every password can only consist of the English letters, both
...
The results only holds if we can guarantee that a user actually chooses a
random password, which is probably not true either. A more realistic setting is
that the attacker uses some dictionary, and that a user's password is not
