...
The server will figure out which folder a user uses for his root folder, this
because it will be the first folder a user creates. In addition it will
probably be the first folder a user creates. The server will also see which
objects the users stores which is a folder and which is a file, because a write
enabler is sent alongside. The sequence of objects that a user requests will
also leak some information about the file and folder hierarchy. We do not
consider any of this information to be dangerous to protect, but if an external
attacker obtain the Write key for a folder this might cause some problems in
...
Alternative cryptographic primitives are also adaptive to the cryptographic
scheme, as long as they provide the necessary cryptographic strength.
...
One could argue both that the proposed scheme does not include authentication,
at the sime time one could argue that the posession of a capability
authenticates a user. If authentication is not implemented as an extra layer
desribed in Section XX, the server does not really authenticate the user.
Granted the knowledge of the Storage Index does indicate possession of the
capability, but this value can theoretically be stolen by using traffic
analysis if the communication channel is not secured.  The posession of a
capability does authentication in some way, but not of the user, rather it
authenticates the capability, and at the same time grants authorization to the
otherwise useless encrypted data.
...
be. This would also be possible with our scheme, but is considered out of
...
enablers for folders can be obtain, which means that the data can also be
