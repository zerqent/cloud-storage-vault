...
free, and available at any time from anywhere \cite{googleservices,
microsoftservices}.
...
application, that fulfils all of the criteria for what we define as a secure
...
fulfil the requirements and characteristics \cite{scrum} of a traditional
...
It should logically not be possible, or at least infeasible, to fake a digital
signature.
...
encryption key and the \ac{IV}, and is transferred to the server.
...
but more and more attacks are discovered %(e.g. by \citet{sha1})
...
client software could implement a time out mechanism for inactive users. If a
user has been inactive for a reasonable given time interval, the user should be
logged out of the application. 

\paragraph{Downloaded Files} Even though a time out value may deny an attacker
