...
primitives. The consumer scenario is depicted in Figure \ref{fig:RW:CCS:CA}. 
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.6]{ArchitectureCCSC.pdf}
    \caption{Cryptographic cloud storage, consumer scenario.}
    \label{fig:RW:CCS:CA}
\end{figure}
...
The Data processor is responsible for encrypting data before it is sent to the cloud, and
decrypting data when it is retrieved from the cloud. This process provides
consumers with confidentiality in the cloud. 

Integrity is supported through a data verifier component. The data verifier checks whether specific
data, stored in the cloud, has been tampered with. The verification procedure is
independent of the download and upload procedures, and can be called at any time
by the consumer. 

The token generator is used by the consumer to generate tokens
that works like data identifiers. Tokens are given to and utilized by the cloud
provider to find data requested by the consumer.

To enable sharing of data stored in the cloud, the architectural scheme is
suggested to use a credential generator. The credential generator is responsible
for generating and sending credentials to other consumers. These credentials are
cryptographic keys that can be used to decrypt defined portions of data stored
in the cloud. The consumer must also send the corresponding tokens together with
the credentials to share data. How tokens and credentials are sent between
consumer is, however, not discussed.

Regarding the encryption scheme, Microsoft propose to utilize attribute based
encryption. In attribute based encryption data is encrypted using a public key
and series of attributes defined as a policy. This ciphertext can further be
decrypted by a set of decryption keys. A decryption key is associated with a set
of attributes, and is able to decrypt the ciphertext if it contains a given
number of the attributes used to encrypt the ciphertext
\cite{microsoftresearch}. The private keys are meant to be implemented as the
distributed credentials mentioned above. It is important to mention that
attribute based encryption is a relatively new technique in cryptography and a
non-standard cryptographic primitive, which makes it hard to define it's level
of security. 
...
arbitrary number of times. The protocol is defined in \cite{proofofstorage}.

The solution for an enterprise scenario is similar to the one of a regular
consumer, however computational components are rather dedicated to separated
...
use the cloud storage application. The distribution of these credentials is
...
\begin{figure}[h!]
    ...
    \label{fig:RW:CCS:EA}
\end{figure}
%TODO: Replace "token and credential"  with "token" in enterprise scenario.
%Two-sided arrows on encrypted data
...
% TODO: Dette skal flyttes PBKDF2
% Her skal vi kun nevne at den smarte usability måten å gjøre dette på er å
% legge capabilities på disken, og at denne må beskyttes i hva vi kaller en
% chain/ring
...
% TODO: Her anbefaler vi 10 tegn, senere anbefaler vi mindre
...
\cite{keystretch}. % TODO: Denne kilden trenger sannsynligvis ikke?
...
% TODO: Dette må henge på greip med evt tidligere 
\begin{itemize}
...
% TODO: Referer til commit/ticket
...
% TODO: samkjør exp_computer med PBKDF2 bruteforce
...
\label{sec:BFLK}
% TODO: Dette er bare sakset hit foreløbig
...
% TODO: Gjøre noe med dette eksempelet
...
%TODO:  Det eneste som er vits her er GPU-griene. Dra også inn ROTH.
