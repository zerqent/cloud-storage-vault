...
We choose to present these papers as they provides possible solutions to the
same problems this thesis is set out to solve, although from a different angle.
One of the goals of the solution 
...
%The \emph{PBKDF2} standard, written by RSA Laboratories in 1999 \cite{PBKDF2std},
%recommends the minimum number of iterations to be 1000.

%Regarding CDA, the use of cloud computing to execute a brute force
...
%To reduce vulnerability from plain brute force attacks, we decided to
...
it's value.
%TODO: 1800 P/s given PBKDF2 with 1000 iterations. Calculate time trivia.
...
%TODO: Calculate some trivia time results
%Given a password with length 5 and containing only small letters, the CDA attack will use 5
%minutes and 39 seconds to check a dictionary containing all possible values. By using a cluster of 200
%nodes the CDA attack is able to check all passwords, in small letters, of length
%6 within 14 minutes and 42 seconds.

%TODO: Calculate some trivia PRICES!
...
%TODO: What dioes the server side know, Information leakage, Weakest link
%Looking at the cryptographic scheme, in a superficially manner, files and folders are
%encrypted locally on a user's client and further stored on a remote server,
%hosted by an untrusted provider.
...
stored data object corresponds to a file or a folder. 
