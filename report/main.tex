...
Considering the weaknesses discussed above, the weakest link in the Cloud
Storage Vault is, by no doubt, the locally encrypted keyring. To provide
...
knowledge. This section will look at the application security in both
scenarios.

\subsubsection{Lost Client}
If a user looses his client and the client is logged on to the Cloud
Storage Vault, anyone who finds the client can get access to the user's private
files and folders. 

While logged on to the service, it is also possible for an
...
user's encrypted files and folders. To reduce this problem the Cloud Storage Vault should implement a
timeout mechanism for inactive users. If a user has been inactive for a given
time interval, the user should be logged out of the application. In this way,
...
and what can be defined as a secure timeout\footnote{Users will not like to write
...
the Cloud. There is, however, two solutions to handle this problem.

The first solution is to avoid writing downloaded files to the client's storage
...
In case limited file size is not an acceptable solution, it is possible to use a temporary filesystem
...
not be used in case users want to store their downloaded files. A second solution would
...
similar to the functionality of Dropbox.
...
enable the attacker to obtain root access on a user's client. With root access,
