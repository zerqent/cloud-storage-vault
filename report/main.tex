...
user experience on an Android device. Potential weaknesses of the system are
identified, and the \emph{weakest link} is derived as to be the password
encrypted keyring stored on the client device.
...
% TODO: Vi må kort beskrive side channel attacks
...
This section describes the low level security primitives used throughout this
thesis.
...
standardized \ac{AES} in 2001. % TODO: KILDE FIPS-197
...
\ac{SHA}-2 family. % TODO: KILDE (NIST). Simpel figur?
...
underlying algorithm. %TODO: Kilde for standard.
...
data. The resulting structure is a directed graph, as illustrated in Figure
\ref{fig:AS:filesystem}.
...
\end{figure}
% TODO: Fix avstand mellom bokser på topp
...
immutable and mutable files respectively.
...
For a symmetric cipher we recommend the \ac{NIST} standard \ac{AES}.  We
recommend a key size of at least 128 bit. \citetalias{ecrypt2_2010}
\cite{ecrypt2_2010} has one of the more pessimistic predictions on how how
secure this choice is, saying data encrypted with this key size should be
secure until 2030-2040. 
...
% TODO: Få fram at man like godt kan velge AES 256
...
been used, but more and more attacks are discovered %(e.g. by \citet{sha1}) and
...
% TODO: Kilde på hvorfor vi ikke kan bruke SHA-1

\subsubsection{Signature Algorithm}

For a signature scheme, the standard seems to be either \ac{RSA} or \ac{DSA}.
Both functions would work, but we recommend \ac{RSA}, primarily because
...
that needs to be transferred, is that of the \emph{root folder}. It is also
...
The construction used to encrypt, hash and upload \textbf{files}, is a pipeline
...
\textbf{Folders} are relatively small in size, and the implementation is not a
...
operations, more specifically the following:
...
the Java function \emph{System.currentTimeMillis()}, which we call before and
...
second, given \emph{PBKDF2} with 4000 iterations. Running the \ac{CDA} in a
cluster of 200 nodes, would further reach a speed of 86 000 passwords per
second. Given a plain brute force attack with a speed of 86 000
password per second, it will take around 2496 years in average to brute force a password
with the password constraints in Section \ref{sec:PBKD}. Calculations are
as follows.
...
believed to offer, what we defined as, a secure storage system. 

Findings in Section \ref{sec:R:BFLK} showed that the presumably weakest link of
the system, the encrypted keyring, is able to withstand heavy brute force
attacks. However, to acquire the keyring, an active attack on the client device
is required. In addition, measuring performance on constrained devices proved
that the trade-off between the levels of usability and security did not
undermine the desired strength of the underlying cryptographic primitives.
...
The scheme does not describe key distribution, so it is also compatible with a
\ac{PKI} in an enterprise setting.
...
Searchable encryption and deduplication are examples of this.
...
open source, proof of concept server and application for Android devices, that
have further been measured in performance and security.
