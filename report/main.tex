...
memory\footnote{tmpfs, ramfs for clients running Unix-like OSs etc.}. However, using only the client
memory to store files, will greatly limit the possible size of the downloaded
...
\subsubsection{Security of the Encrypted Keyring}
In both cases above, with a compromised or lost user client, an attacker can
copy/achieve the encrypted keyring stored by the Cloud Storage Vault. The
application's security is therefore dependent on the cryptographic strength and
brute force resistance of its keyring.
