...
utilized to implement an Android application, to test whether or not it provided
...
The work behind this report was carried out during the spring semester
in 2011 at the Norwegian University of Science and Technology (NTNU), Department
of Telematics (ITEM). The report is a final result from a Master thesis in
information security written by three graduate students at NTNU. The
thesis was assigned by Earnst \& Young AS Norway, and acknowledged by ITEM.

We would like to thank our external supervisors Carsten Maartmann-Moe and
Antonio Martiradonna, at Earns \& Young AS, for their valuable contributions.
Additionally, we would like to thank Danilo Gligorosky at the Department of
Telematics for providing constructive feedback on the report and for giving us the opportunity
...
software indeed does what stated.

\paragraph{Network Scheme} Wuala boasts strong focus on reliability and
...
the system. There are also some other advantages coming directly from using a
distribution scheme based on \ac{P2P}, as no maximum file size or traffic
limit.
...
Though, none of these security features can be verified, since the source is
not available for scrutiny by the public.
...
As it turns out, one of the greatest challenges when developing a secure cloud
file system, is the choice and implementation of the cryptographic primitives.
Good references and external recommendations can help, but only so far. Key
distribution is also a problem that will always be hard to accomplish if a
trusted third party is not included. Another challenge experienced is that all
features one would want from a secure cloud file system are hard to implement
without compromising security (searchable encryption, deduplication).

This thesis propose a scheme for secure cloud storage based on Tahoe-\ac{LAFS},
it also encompass an open source, free, implementation of this scheme, that
works satisfiable on a constrained device, an Android smartphone.

While our scheme solves some of the basic challenges of secure cloud storage,
there are several features which would improve the scheme. The most important
