...
public class CryptoutilTest {
	...
	public void testSigning() {
		...
		Assert.assertTrue(Cryptoutil.signature_valid(sign, hashed, pair.getPublic()));

		KeyPair pair2 = Cryptoutil.generateAsymmetricKeys(); 
		Assert.assertFalse(Cryptoutil.signature_valid(sign, hashed, pair2.getPublic()));
	}
	...
	public void testRSAKeyGeneration() {
		...
		// We need keys to be of a static size //TODO: Figure out why PrivateExponent.length is sometimes 129
		for (int i = 0; i < TEST_AMOUNT; i++) {
			...
			Assert.assertEquals(128, priv.getPrivateExponent().toByteArray().length);
	...
	public void testRSASerialization() {
		...
		RSAPrivateKey serPriv = (RSAPrivateKey)Cryptoutil.createRSAPrivateKey(b_priv);
		RSAPublicKey serPub = (RSAPublicKey)Cryptoutil.createRSAPublicKey(b_pub);

		Assert.assertArrayEquals(rsaPriv.getModulus().toByteArray(), serPriv.getModulus().toByteArray());
		Assert.assertArrayEquals(rsaPub.getModulus().toByteArray(), serPub.getModulus().toByteArray());
		Assert.assertArrayEquals(rsaPriv.getModulus().toByteArray(), serPub.getModulus().toByteArray());
		Assert.assertArrayEquals(rsaPub.getModulus().toByteArray(), serPriv.getModulus().toByteArray());

		Assert.assertArrayEquals(rsaPriv.getPrivateExponent().toByteArray(), serPriv.getPrivateExponent().toByteArray());
		Assert.assertArrayEquals(rsaPub.getPublicExponent().toByteArray(), serPub.getPublicExponent().toByteArray());
	...
	@Test
	public void testThatRSASerializationWillWork() throws NoSuchAlgorithmException, InvalidKeySpecException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException {
